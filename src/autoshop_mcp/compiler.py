    ...
    with tempfile.TemporaryDirectory(prefix="as-native-") as tmp:
        ...
        try:
            ...
            for number in range(1, len(blocks) + 3):
                ...
                proc = subprocess.run(cmd, cwd=runtime["install_dir"], capture_output=True,
...
def ld_to_il_copy(project: str, file: str, dest: str, install_dir: str | None = None) -> dict:
    ...
    if not item or item["file_type"] != 1 or Path(file).suffix.lower() != ".ld":
        raise ToolError("unsupported_pou_type", "只能转换工程索引中已登记的 LD 程序块。")
    new_name = str(Path(file).with_suffix(".IL"))
    if (src / new_name).exists():
        raise ToolError("destination_exists", "同名 IL 已存在，拒绝覆盖。")
    before = inventory(src)
    out.mkdir(parents=True)
    report = {"ok": False, "tool": "ld_to_il_copy", "source": str(src), "file": file,
    ...
    with tempfile.TemporaryDirectory(prefix="as-convert-") as tmp:
        ...
        try:
            ...
            cmd = [str(NATIVE / "host.exe"), str(NATIVE / "autoshop-native.manifest"), str(work),
                   runtime["install_dir"], str(decoded), meta["index_file"], str(item["id"])]
            proc = subprocess.run(cmd, cwd=runtime["install_dir"], capture_output=True,
                                  timeout=30, creationflags=subprocess.CREATE_NO_WINDOW)
            ...
            if proc.returncode or "convert_finished w=1 " not in log or not (work / new_name).is_file():
                raise ToolError("native_conversion_failed", "原厂 LD 转 IL 未成功。")
            ...
            def replace(match):
                body = match[2]
                if body.count("<FileType>1</FileType>") != 1 or body.count("<FileName>" + file + "</FileName>") != 1:
                    raise ToolError("unsupported_index_layout", "索引字段格式不匹配。")
                return match[1] + body.replace("<FileType>1</FileType>", "<FileType>0</FileType>").replace(
